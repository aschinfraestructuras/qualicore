...
import { Obra, ZonaObra, FaseObra, EquipaObra, RiscoObra, IndicadorObra } from '../../types'
import { Plus, X, Building, MapPin, Calendar, Euro, Users, AlertTriangle, BarChart3 } from 'lucide-react'
...
export default function ObraForm({ initialData, onCancel, onSubmit }: ObraFormProps) {
  ...
  // Adicionar fase
  const addFase = () => {
    const novaFase: FaseObra = {
      id: Date.now().toString(),
      nome: '',
      descricao: '',
      ordem: (formData.fases || []).length + 1,
  ...
  // Atualizar fase
  const updateFase = (index: number, field: keyof FaseObra, value: any) => {
    setFormData(prev => ({
      ...prev,
  ...
  const addEquipa = () => {
    ...
    setFormData(prev => ({
      ...prev,
      equipas: [...(prev.equipas || []), novaEquipa]
    }))
  }

  // Remover equipa
  const removeEquipa = (index: number) => {
    setFormData(prev => ({
      ...prev,
      equipas: (prev.equipas || []).filter((_, i) => i !== index)
    }))
  }

  // Atualizar equipa
  const updateEquipa = (index: number, field: keyof EquipaObra, value: any) => {
    setFormData(prev => ({
      ...prev,
      equipas: (prev.equipas || []).map((e, i) =>
  ...
  // Atualizar risco
  const updateRisco = (index: number, field: keyof RiscoObra, value: any) => {
    setFormData(prev => ({
      ...prev,
      riscos: (prev.riscos || []).map((r, i) =>
  ...
  // Adicionar indicador
  const addIndicador = () => {
    const novoIndicador: IndicadorObra = {
      id: Date.now().toString(),
      nome: '',
      tipo: 'qualidade',
      unidade: '',
      ...
      responsavel: '',
      data_ultima_medicao: new Date().toISOString().split('T')[0],
      historico: [],
      status: 'dentro_meta'
    }
    ...
    setFormData(prev => ({
      ...prev,
      indicadores: [...(prev.indicadores || []), novoIndicador]
    }))
  }
  ...
  // Atualizar indicador
  const updateIndicador = (index: number, field: keyof IndicadorObra, value: any) => {
    setFormData(prev => ({
      ...prev,
      indicadores: (prev.indicadores || []).map((i, idx) =>
