...
const menuItems = [
  ...
  { path: '/fornecedores', icon: Truck, label: 'Fornecedores', description: 'Cadastro de fornecedores', color: 'from-cyan-500 to-cyan-600' },
  { path: '/nao-conformidades', icon: AlertTriangle, label: 'Não Conformidades', description: 'Gestão de NCs', color: 'from-red-500 to-red-600' },
  { path: '/relatorios', icon: BarChart3, label: 'Relatórios', description: 'Análises e relatórios', color: 'from-violet-500 to-violet-600' },
]
...
const quickActions = [
  ...
  { path: '/nao-conformidades/nova', icon: AlertTriangle, label: 'Reportar NC', color: 'from-red-500 to-red-600' },
]
    ...
    <nav className="w-full bg-gradient-to-r from-blue-600 via-blue-500 to-blue-600 shadow-lg px-6 py-0 flex flex-col md:flex-row md:items-center md:justify-between z-40 relative">
      ...
      <div className="flex items-center justify-between py-3">
        <Link to="/" className="flex items-center space-x-3 group">
          ...
          <span className="text-xl font-bold text-white font-display tracking-wide group-hover:text-blue-100 transition">Qualicore</span>
        </Link>
      ...
      <div className="hidden md:flex items-center space-x-6 py-2">
        ...
        <div className="relative">
          ...
          >
            ...
            Ações Rápidas
            <ChevronDown className={`h-4 w-4 ml-2 transition-transform duration-200 ${showQuickActions ? 'rotate-180' : ''}`} />
          </button>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-white truncate">José Antunes</p>
            <p className="text-xs text-blue-200 truncate">Engenheiro de Qualidade</p>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-white/20 py-4 space-y-4">
          ...
          <div className="border-t border-white/20 pt-4">
            <h3 className="text-xs font-semibold text-blue-200 uppercase tracking-wider mb-3 px-4">Ações Rápidas</h3>
