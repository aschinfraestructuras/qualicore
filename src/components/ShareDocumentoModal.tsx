...
interface ShareDocumentoModalProps {
  isOpen: boolean;
  onClose: () => void;
  documento: Documento;
}

export const ShareDocumentoModal: React.FC<ShareDocumentoModalProps> = ({
  isOpen,
  onClose,
  documento,
}) => {
  const [email, setEmail] = useState("");
  const [subject, setSubject] = useState(`Documento: ${documento.codigo}`);
  const [message, setMessage] = useState(
    `Segue em anexo o documento ${documento.codigo} - ${documento.tipo}.\n\nDetalhes:\n- Código: ${documento.codigo}\n- Tipo: ${documento.tipo}\n- Versão: ${documento.versao}\n- Responsável: ${documento.responsavel}\n- Estado: ${documento.estado}\n- Zona: ${documento.zona}\n\nCumprimentos,`
  );
  ...
  const handleShareByEmail = async () => {
    ...
    setIsLoading(true);
    try {
      await shareService.shareDocumentoByEmail(documento, email, subject, message);
      toast.success("Documento partilhado por email com sucesso!");
      onClose();
    } catch (error) {
      console.error("Erro ao partilhar documento:", error);
      toast.error("Erro ao partilhar documento por email");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveToBackend = async () => {
    setIsLoading(true);
    try {
      await shareService.saveDocumentoToBackend(documento);
      toast.success("Documento guardado no backend com sucesso!");
      onClose();
    } catch (error) {
      console.error("Erro ao guardar documento:", error);
      toast.error("Erro ao guardar documento no backend");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDownloadLocally = async () => {
    setIsLoading(true);
    try {
      await shareService.downloadDocumentoLocally(documento);
      toast.success("Documento descarregado localmente com sucesso!");
      onClose();
    } catch (error) {
      console.error("Erro ao descarregar documento:", error);
      toast.error("Erro ao descarregar documento localmente");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="p-6 border-b bg-gray-50">
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600 capitalize">{documento.tipo}</p>
