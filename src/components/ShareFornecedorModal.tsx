...
interface ShareFornecedorModalProps {
  isOpen: boolean;
  onClose: () => void;
  fornecedor: Fornecedor | null;
}

export const ShareFornecedorModal: React.FC<ShareFornecedorModalProps> = ({
  isOpen,
  onClose,
  fornecedor
}) => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const results = await shareService.shareFornecedor(fornecedor, options);
      setResults(results);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-4">
          <h3 className="font-medium text-gray-800 mb-2">Fornecedor</h3>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div><span className="font-medium">Nome:</span> {fornecedor.nome}</div>
            <div><span className="font-medium">NIF:</span> {fornecedor.nif}</div>
            <div><span className="font-medium">Estado:</span> {fornecedor.estado}</div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder={`Fornecedor ${fornecedor.nome}`}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
