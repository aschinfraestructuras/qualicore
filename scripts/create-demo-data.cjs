...
const demoData = {
  ...
  materiais: [
    {
      nome: "Betão C25/30",
      tipo: "Betão",
      especificacoes: "Betão para fundações",
      fornecedor_id: null,
    },
  ],
  ensaios: [
  ...
  documentos: [
    {
      ...
      estado: "aprovado",
      descricao: "Projeto estrutural do edifício.",
    },
  ],
  rfis: [
    {
      numero: "RFI-001",
      titulo: "Dúvida sobre fundações",
      descricao: "Qual a profundidade das sapatas?",
      solicitante: "Eng. João",
      prioridade: "media",
      estado: "pendente",
      impacto_custo: "baixo",
      impacto_prazo: "nenhum",
    },
  ],
  checklists: [
    {
  ...
  nao_conformidades: [
    {
      titulo: "NC Demo",
      descricao: "Fissura na viga",
      gravidade: "media",
      estado: "aberta",
      acoes_corretivas: "Reparar viga",
    },
  ],
};
    ...
    for (const [colecao, registos] of Object.entries(demoData)) {
      ...
      for (const registo of registos) {
        try {
          ...
          console.log(
            `ℹ️ Registo demo já existe ou erro em ${colecao}:`,
            error.message,
          );
        }
      }
