...
async function setupCollections() {
  try {
    ...
    if (authResponse.status !== 200) {
      ...
      const authResponse2 = await makeRequest(
        `${POCKETBASE_URL}/api/admins/auth`,
        {
          method: "POST",
          body: {
            email: ADMIN_EMAIL,
      ...
      if (authResponse2.status !== 200) {
        throw new Error(
          "Falha na autenticação: " + JSON.stringify(authResponse2.data),
        );
      }
    ...
    async function createCollection(name, schema) {
      ...
      if (res.status === 200) {
        ...
        res.data?.code === 400 &&
        res.data?.data?.name?.code === "validation_collection_name_exists"
      ) {
    ...
    await createCollection("obras", [
      ...
      },
      {
        ...
        type: "select",
        options: {
          values: ["residencial", "comercial", "industrial", "infraestrutura"],
        },
      },
