-- Migracao completa para criar as tabelas obras e ensaios
-- Execute este script no SQL Editor do Supabase

-- 1. CRIAR TABELA OBRAS (se nao existir)
...
-- Politicas para obras
CREATE POLICY "Usuarios podem ver suas proprias obras" ON obras
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Usuarios podem inserir suas proprias obras" ON obras
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Usuarios podem atualizar suas proprias obras" ON obras
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Usuarios podem deletar suas proprias obras" ON obras
    FOR DELETE USING (auth.uid() = user_id);

-- Politicas para ensaios
CREATE POLICY "Usuarios podem ver seus proprios ensaios" ON ensaios
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Usuarios podem inserir seus proprios ensaios" ON ensaios
    FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Usuarios podem atualizar seus proprios ensaios" ON ensaios
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Usuarios podem deletar seus proprios ensaios" ON ensaios
    FOR DELETE USING (auth.uid() = user_id);
...
VALUES (
    ...
    'Ensaio de resistencia a compressao do concreto',
    '35 MPa',
    'aprovado',
    true,
    'Laboratorio Tecnico',
...
SELECT 'Migracao concluida com sucesso! Tabelas obras e ensaios criadas.' as status; 