...
const modulesToCheck = [
  ...
  { file: 'src/pages/Fornecedores.tsx', api: 'fornecedoresAPI', name: 'Fornecedores' },
  { file: 'src/pages/Documentos.tsx', api: 'documentosAPI', name: 'Documentos' },
  { file: 'src/pages/Ensaios.tsx', api: 'ensaiosAPI', name: 'Ensaios' },
  { file: 'src/pages/Checklists.tsx', api: 'checklistsAPI', name: 'Checklists' },
  { file: 'src/pages/RFIs.tsx', api: 'rfisAPI', name: 'RFIs' },
  { file: 'src/pages/Materiais.tsx', api: 'materiaisAPI', name: 'Materiais' },
  { file: 'src/pages/NaoConformidades.tsx', api: 'naoConformidadesAPI', name: 'Não Conformidades' }
];
  ...
  try {
    ...
    if (usesCorrectAPI && !usesForbiddenAPI) {
      ...
      if (usesForbiddenAPI) {
        ...
        issues.push(`${module.name}: Usa APIs proibidas: ${foundForbidden.join(', ')}`);
      }
    }
...
console.log(`Percentual de sucesso: ${Math.round((correctModules / totalModules) * 100)}%`);
...
if (issues.length > 0) {
  ...
  if (issues.some(issue => issue.includes('naoConformidadesAPI'))) {
    console.log('3. Corrigir Não Conformidades: Implementar naoConformidadesAPI');
  }
